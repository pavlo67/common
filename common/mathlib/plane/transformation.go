...
func CalculateRotationAndScale(pts []Transformation, rotDeviationMax, scaleDeviationMax float64) (LeftAngle, float64, error) {
	if len(pts) != 3 {
	...
	var rotDeltaSum LeftAngle
	var scalesSum float64

	rotDeltas := make([]LeftAngle, len(pts))
	...
	rotDeltaAvg := (rotDeltaSum / LeftAngle(len(pts))).Canon()
