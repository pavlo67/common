...
//func Filter(userIS auth.ID, grpsOp groups.Operator, linksList []Item) (linksListFiltered []Item) {
...
//func SelectString(userIS auth.ID, linksOp Operator, linkType string) (selects viewshtml.SelectString) {
...
//func Correct(is auth.ID, linksListTmp []Item) (linksList []Item) {
//	for _, l := range linksListTmp {
//		if l.ROwner == is { // l.Type == TypeTag &&
//			l.Name = strlib.ReSpaces.ReplaceAllString(strings.TrimSpace(l.Name), " ")
