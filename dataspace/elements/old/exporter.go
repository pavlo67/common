...
func ImportTo(userIS auth.ID, objectsOp Operator, id, status string) error {
...
func Export(user *auth.User, objectsOp Operator, credentialsOp users.Operator, ctrl groups.Operator, filesOp files.Operator, selector selectors.Selector) (string, error) {
	...
	//options := crud.ReadOptions{Selector: selector, ForExport: true}
...
func Import(userIS auth.ID, zipName string, objectsOp Operator, credentialsOpOp users.Operator, ctrl groups.Operator, groupsOp groups.Operator, filesOp files.Operator, overwrite, unknownAsMine bool) error {
...
func JSONToObjects(userIS auth.ID, filesOp files.Operator, credentialsOp users.Operator, ctrl groups.Operator, groupOp groups.Operator, zipName string, unknownAsMine bool) ([]notes.Item, string, error) {
...
func setLocalISRights(o *notes.Item, isForGlobalIS map[string]auth.ID) {
...
func setLocalRight(rg, r auth.ID) auth.ID {
	if rg == "" {
		return ""
	}
	if r != "" {
...
//			//	gr := groups.Census{
//			//		IdentityNamed: auth.IDentityNamed{
//			//			Nick: i.Name,
//			//		},
//			//		GlobalIS: i.GlobalIS,
//			//	}
...
func FilesToRepository(userIS auth.ID, filesOp files.Operator, importDir string, linksList, oldLinks []notes.Item) ([]notes.Item, error) {
...
func setGlobalISRights(o *notes.Item, identityGlobalIS map[auth.ID]exportIdentity) {
...
func setGlobalRight(r auth.ID, rg string) auth.ID {
	if rg != "" {
		return auth.ID(rg)
	} else {
...
func getAllIdentitiesFromObject(user *auth.User, identityGlobalIS map[auth.ID]exportIdentity, credentialsOpOp users.Operator, ctrl groups.Operator, rView, rOwner auth.ID, managers rights.Managers) map[auth.ID]exportIdentity {
...
func setGlobalIS(user *auth.User, is auth.ID, credentialsOp users.Operator, ctrl groups.Operator) (string, string) {
	//if is == basis.Anyone {
	//	return string(is), ""
	//}
	//var err error
